...
public class Rule {
	private static int lable_counter = 0;
		...
		public Node(SLRItem item, List<TreeNode> childs) {
			...
			case "S":
				if (equal(item.right, new String[] { Tag.ID, Operator.ASSIGN, "E", Operator.LNND })) {
					...
					//childs.get(2).setProperty("true", genLabel());
					//childs.get(2).setProperty("false", genLabel());
					next=genLabel();
					...
					//String label_inter=genLabel();
					//childs.get(0).setProperty("next", label_inter);
					next=childs.get(1).getValue("next");
					//childs.get(1).setProperty("next", getValue("next"));
					
					//code = childs.get(0).getValue("code") + label_inter + childs.get(1).getValue("code");
	...
	private static String genLabel() {
		return "label" + (lable_counter++) + ":\n";
	}

	private static String genTmp() {
		return "tmp" + (tmp_counter++);
	}
}
